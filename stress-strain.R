...
ggplot(data = filter(pH_crushes.Kenya, pH == "6"),
       #pH_crushes,
       aes(x= psi, y = delta_frac.area,
           group = interaction(pH, larva, sac),
           #group = as_factor(pH),
           colour = as_factor(pH)), na.rm = F) +

  geom_smooth(data = filter(pH_crushes.Kenya, pH == 6), method = 'lm',
